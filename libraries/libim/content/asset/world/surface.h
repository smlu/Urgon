...
namespace libim::content::asset {
    ...
    struct Surface final : public Face
    {
        ...
        {
            None                     = 0x0,
            Floor                    = 0x1,        // could be face collision if Collision is set
            CogLinked                = 0x2,
            Collision                = 0x4,        // Surface has collision
            AiCannotWalkOnFloor      = 0x8,        // Ai won't walk on this surface
            DoubleSurfaceScale       = 0x10,       // when set, it affects SlideWall() cog function. By default the animating surface matrix is resized to 320x320, this flag resizes it to 640x640
            HalfSurfaceScale         = 0x20,       // when set, it affects SlideWall() cog function. By default the animating surface matrix is resized to 320x320, this flag resizes it to 160x160
            EighthSurfaceScale       = 0x40,       // when set, it affects SlideWall() cog function. By default the animating surface matrix is resized to 320x320, this flag resizes it to 40x40
            ...
            Earth                    = 0x80000,
            Web                      = 0x100000,
            Lava                     = 0x200000,   // also 02_riv freezing water has this flag set
            Snow                     = 0x400000,
            Wood                     = 0x800000,
            ...
            QuaterSurfaceScale       = 0x4000000,  // when set, it affects SlideWall() cog function. By default the animating surface matrix is resized to 320x320, this flag resizes it to 80x80
            QuadrupleSurfaceScale    = 0x8000000,  // when set, it affects SlideWall() cog function. By default the animating surface matrix is resized to 320x320, this flag resizes it to 1280x1280. example: in 00_cyn.ndy water surfaces
